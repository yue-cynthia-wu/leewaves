...
REAL(kind=rc_kind)      :: u_taub, z0s_gotm, z0b_gotm    ! z0b_gotm= roughness length, 
...
charnockval = 1400.d0; clip = 1.d-3; MaxItz0b = 1;
...
!! Update diffusivities in PSOM
   num(k)= dmax1(1.d-5,num(k))
   KzMom(i,j,k)= dmin1(num(k),clip)

   nuh(k)= dmax1(1.d-5,nuh(k))
   KzTr(i,j,k)= dmin1(nuh(k),clip)
