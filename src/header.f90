...
#include "size.h"                                 ! Grid size and array sizes for mgrid.
 ...
 !-- physical properties
 !----------------------
  REAL(kind=rc_kind), PARAMETER :: S0=35.7d0, T0=15.d0 ,R0=1027.d0  !@ S0: mean salinity, T0: mean temperature, R0: mean density
 ...
 !----------------------------
  ...
  REAL(kind=rc_kind) :: restore                              !@ restore lapse rate
 ...
 !------------------------
  ...
  REAL(kind=rc_kind) :: dztop_dim                      !@ dztop_dim  : depth of the top layer, dimensional (in m), in namelist
...
!------------------------------------------------------------------------!
  ...
  REAL(kind=rc_kind), dimension(    0:NI+1,0:NJ+1, 0:NK+1    ) :: s_ini, T_ini                          ! @ initial s,T
  REAL(kind=rc_kind), dimension(    0:NI+1,0:NJ+1, 0:NK+1    ) :: s_restore, T_restore                  ! @ restore s,T
