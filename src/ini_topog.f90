...
#include "cppdefs.h"
  ...
  !     fits a spline to the described topog and computed Ddx(i)          
      integer i,j, nwaves
      REAL(kind=rc_kind) :: dep,depmean,amplitude,LbyD,xlen,xmeter,approx_wavelength,wavelen,deriv

      depmean= 2000.   ! total depth  
      amplitude = 0.5*100. ! 1/2 the amplitude of topography
      approx_wavelength = 5000 ! wavelength in meters
      xlen= dx*dble(NI)   ! length of channel (x-dimension) in meters
      nwaves = xlen / approx_wavelength   ! nwaves is integer = the number of wavelengths that fit in the channel 
      wavelen = xlen / nwaves 
...
!         dep= depmean*(1. + 0.1 *dsin(10.*2.*pi*xc(i)/xlen))
!         deriv =  0.1*depmean * dcos(10.*2.*pi*xc(i)/xlen)*( 10.*2.*pi/xlen)
         dep= -depmean - amplitude* dsin(2.*pi*xmeter/wavelen)
         deriv=  -amplitude* (2.*pi/wavelen)* dcos(2.*pi*xmeter/wavelen)
         ! write(6,*) j,yc(j),dep                                       
