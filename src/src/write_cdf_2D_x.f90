subroutine write_cdf_2D_x(islice,counter_2d,n)
  ...
  !     ------------------------------------------------------------      
  USE header,ONLY : NI,NJ,NK,ntr,nconsume, s,T,rho,u,v,w,consump,Tr,KzMom,KzTr,shear,vor,freqN2,yc,zc,fricb,stress_top_x,DL,time_seconds,time_days,dirout,rc_kind, pvt, rp, stress_top_y, pv3, swr,qloss
...
!         dimstim = ncddef(idDatFile,'time',NCUNLIM,rcode) 
         ...
         idvswr= ncvdef(idDatFile,'swr',NCFLOAT,3,dimswind,rcode) 
         idvqloss= ncvdef(idDatFile,'qloss',NCFLOAT,3,dimswind,rcode) 
         idvcon    = ncvdef(idDatFile,'consump',NCFLOAT,4,dims4c,rcode) 
         idvtr = ncvdef(idDatFile,'tr',NCFLOAT,4,dims4,rcode) 
        ! idvtbar = ncvdef(idDatFile,'trbar',NCFLOAT,4,dims4,rcode) 
         idvs   = ncvdef(idDatFile,'s',NCFLOAT,4,dims,rcode)          
         ...
         idvKzMom = ncvdef(idDatFile,'KzMom',NCFLOAT,4,dims,rcode) 
         idvKzTr = ncvdef(idDatFile,'KzTr',NCFLOAT,4,dims,rcode) 
         !idvfb = ncvdef(idDatFile,'fricb',NCFLOAT,4,dims,rcode) 
         idvpvt = ncvdef(idDatFile,'pvt',NCFLOAT,4,dims,rcode) 
         idvpv3 = ncvdef(idDatFile,'pv3',NCFLOAT,4,dims,rcode) 
         ...
         idvbysection = ncvdef(idDatFile,'by',NCFLOAT,4,dims,rcode) 
