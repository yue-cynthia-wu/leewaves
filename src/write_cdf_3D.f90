...
!----------------------------------------------------------             
  ...
  USE header, only : NI,NJ,NK,xc,yc,zc,p,h,consump,T,s,rho,Tr,u,v,w,vor,pv,uf,vf,wf,Kz,conv,con100,nconsume,dirout,rc_kind,ntr,Jac
#include "netcdf.inc"                                                                        
  ...
  integer start(3), count(3), start2d(2), count2d(2),count2dtr(3),count4(4),start4(4),start2dtr(3),count4consump(4), countdiv(3)
  integer countuf(3), countvf(3), countwf(3)

  integer dims(3), dims2d(2),dims2dtr(3),dims4(4),dimsconsump(4), dimsdiv(3), dimfdiv(1)   !! ADDED 
  ...
  integer :: idvptfx, idvptfy, idvptfz, idvJac, idvptc, idvdum1, idvdum2, idvdum3, idvfdiv !! ADDED           
...
!   dimsconsump(3) = dims(3); dimsconsump(4) = ncddef(idDatFile,'ntrcon',nconsume,rcode);
  ...
  ! 3D dimensions for divergence
  countdiv(1)= NI; countdiv(2)= NJ; countdiv(3)= NK; !! ADDED
!   dimsdiv(1) = ncddef(idDatFile,'dum1',NI,rcode);    !! ADDED
!   dimsdiv(2) = ncddef(idDatFile,'dum2',NJ,rcode);    !! ADDED
!   dimsdiv(3) = ncddef(idDatFile,'dum3',NK,rcode);    !! ADDED
...
!   idvdum1 = ncvdef(idDatFile,'dum1',NCDOUBLE,3,dimsdiv,rcode) !! ADDED
!   idvdum2 = ncvdef(idDatFile,'dum2',NCDOUBLE,3,dimsdiv,rcode) !! ADDED
!   idvdum3 = ncvdef(idDatFile,'dum3',NCDOUBLE,3,dimsdiv,rcode) !! ADDED
...
!   CALL ncvpt(idDatFile,idvdum1, start, countdiv, dum1, rcode) !! ADDED
