...
! uvarx_advec is not needed
  ...
  if(av_comp(selvar)==1) then
    ...
    ! Advection and horizontal diffusion are done - both are in uvarx

!  uvarx_advec is not really needed.  Will eliminate it later
...
!      iv_compute_kz=0;
    ...
    ! uvarx now has the divergence of the diffusive flux plus source terms
    ! ---------------------------------------------------------------
