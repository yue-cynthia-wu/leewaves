...
REAL(kind=rc_kind), dimension(      NI  ,  NJ  ,   NK  )  :: dum1,dum2,dum3
...
!   dwfpdz= wf(i,j,k)*ptfz(i,j,k)-wf(i  ,j  ,k-1)*ptfz(i  ,j  ,k-1)
	...
	dufpdx= uf(i+1,j,k)*ptfx(i+1,j,k)-uf(i,j,k)*ptfx(i,j,k)
	dvfpdy= vf(i,j+1,k)*ptfy(i,j+1,k)-vf(i,j,k)*ptfy(i,j,k)
	dwfpdz= wf(i,j,k+1)*ptfz(i,j,k+1)-wf(i,j,k)*ptfz(i,j,k)
...
!   	dwfdz= (wf(i,j,k)-wf(i  ,j  ,k-1))
  	...
  	dufdx= (uf(i+1,j,k)-uf(i,j,k))
  	dvfdy= (vf(i,j+1,k)-vf(i,j,k)) 
  	dwfdz= (wf(i,j,k+1)-wf(i,j,k)) 
...
!   wdpdz=0.5*(wf(i,j,k)+wf(i  ,j  ,k-1))*(ptfz(i,j,k)-ptfz(i  ,j  ,k-1))
	...
	udpdx=0.5*(uf(i+1,j,k)+uf(i,j,k))*(ptfx(i+1,j,k)-ptfx(i,j,k))
	vdpdy=0.5*(vf(i,j+1,k)+vf(i,j,k))*(ptfy(i,j+1,k)-ptfy(i,j,k))
	wdpdz=0.5*(wf(i,j,k+1)+wf(i,j,k))*(ptfz(i,j,k+1)-ptfz(i,j,k))
